...
router.post("/", async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    // -------------------------------
    const { c_name, c_phone, c_email, c_country, c_state } = req.body;
    ...
    if (typeof c_email === "string") {
      emailArray = c_email.split(",").map((e: string) => e.trim()).filter(Boolean);
    } else if (Array.isArray(c_email)) {
      emailArray = c_email.flatMap((e: string) => e.split(",")).map((e: string) => e.trim()).filter(Boolean);
    }
    ...
    // -------------------------------
    // 2️⃣ Check for duplicate primary email
    // -------------------------------
    const primaryEmail = emailArray[0].toLowerCase();
    const existingCustomer = await Client.findOne({
      c_email: { $elemMatch: { $regex: new RegExp(`^${primaryEmail}$`, "i") } },
    });

    if (existingCustomer) {
      return res.status(400).json({ success: false, error: "Primary email already exists." });
    }

    // -------------------------------
    // 3️⃣ Password generation & encryption
    // -------------------------------
    const plainPassword = req.body.password?.trim() || (Math.random().toString(36).slice(-8) + crypto.randomBytes(2).toString("hex"));
    const salt = await bcrypt.genSalt(10);
    await bcrypt.hash(plainPassword, salt); // hashed if needed
    const key = crypto.createHash("sha256").update(process.env.ENCRYPTION_SECRET || "default_secret").digest();
    ...
    // 5️⃣ Create Customer
    // -------------------------------
    const customer = await Client.create(customerData);
